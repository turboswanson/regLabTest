...
QInstaller::QInstaller(QWidget* parent) : QWidget{parent}
{
    ...
    _topWidget->setStyleSheet("#topWidget {"
    ...
    connect(_bottomWidget, &Navigator::leftButtonClicked,
            [this]() { _topWidget->prevPage(); });
    connect(_bottomWidget, &Navigator::rightButtonClicked, [this]() {
    ...
    connect(_topWidget, &PageController::pageChanged, this,
            &QInstaller::handlePageChanged);
    connect(_topWidget, &PageController::packageSelected, this,
            &QInstaller::handlePackageSelected);
}
...
void QInstaller::handleInstallation(const Packages& selectedPackage)
{
    ...
    {
    case Packages::PIP:
        cmd = "python3 -m pip instal --user requests";
        break;
    case Packages::NPM:
    ...
    connect(proc, &QProcess::readyRead, this, [proc]() { qDebug() << proc->readAll(); });

    connect(proc, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
            [this, proc, selectedPackage](int code, QProcess::ExitStatus status) {
                qDebug() << "Finished with code:" << code << "_status:" << status;
...
void QInstaller::handleBusyScreen()
{
    ...
    connect(this, &QInstaller::installationFinished, dialog,
            [this, dialog, label, okButton](const int& code) {
                const bool success = (static_cast<EXIT_CODE>(code) == EXIT_CODE::OK);

                QString message =
                    success
                        ? QString("%1 installation completed successfully!")
                              .arg(packageToString(static_cast<Packages>(_selectedIndex)))
                        : QString("%1 installation failed.")
                              .arg(
                                  packageToString(static_cast<Packages>(_selectedIndex)));
